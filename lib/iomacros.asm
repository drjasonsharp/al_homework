...
;;
section	.data
__format_str:	db	"%s",0,0
__format_int:	db	"%d",0,0
__format_hex:	db	"%#010x",0,0
__format_f:	db	"%f",0,0
__format_dbl:	db	"%lf",0
__open_read:	db	"r",0,0,0
__open_write:	db	"w",0,0,0
...
%ifidni %2,rax
%error fget_str uses RAX to provide return status, so it may not be an argument
%endif
	push	rbx		; save context
	push	rcx
	push	rdx
...
;; fdump
;;---------
;; Displays the contents of the FPU register file ... without messing
;; anything up.  This is helpful in debugging programs.
;;-----------------------------------------------------------
;%macro	fdump	0
;	push	RAX
;	push	RBX
;	push	RSI
;	fsave	[__fpu_mem]
;	frstor	[__fpu_mem]		; we restore immediately
;	mov	RSI,8			; b/c fsave apparently does
;  	mov	RBX,0x30		; an finit
;
;%%loop:	sub	RSP,8
;	fstp	qword [RSP]
;	put_ch	0x53			; display labels: STx_
;	put_ch	0x54
;	put_ch	BL
;	put_ch	0x20
;	push	qword __format_dbl	; format string is a param
;	call	printf
;	add	RSP,12
;	put_ch	10
;	inc	RBX
...
;	jmp	%%loop
;
;%%done:	frstor	[__fpu_mem]		; we re-restore here, too.
;	pop	RSI
;	pop	RBX
;	pop	RAX
;
